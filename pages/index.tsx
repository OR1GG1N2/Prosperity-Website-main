		...
		<>
			...
			<main id="main" className="markdown">
				<section className="flex flex-col gap-12 text-center markdown my-52">
					...
					<header className="text-6xl type-header">Строй, твори и создавай!</header>
					<p className="text-lg text-zinc-300 font-medium max-w-2xl mx-auto">
					Добро пожаловать в Minecamp — мир, где вас встречает уют и дружелюбие.
					{/* <H>Create</H>, <H>engineer</H>, and <H>grow</H> on a server you can call home. */}
					</p>
					<div className="space-x-12">
						<A href="https://origg.in/discord" noIcon={true}>
							<button className="bg-sky-500 hover:bg-sky-400 duration-150 text-white px-14 py-2 type-header rounded justify-items-end">
								Зайти в Discord 
								<ArrowRightIcon className="w-5 h-5 inline-block align-middle" />
							</button>
						</A>
						<A href="/wiki/join-info" className="hidden md:inline custom-link type-header">
							Как подключиться? <BookOpenIcon className="w-5 h-5 inline-block align-middle" />
						</A>
					</div>
				</section>
